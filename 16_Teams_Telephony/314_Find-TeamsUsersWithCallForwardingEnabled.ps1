<#
.SYNOPSIS
    Teams Users With Call Forwarding Enabled

.DESCRIPTION
    Identifiziert Teams Users With Call Forwarding Enabled in der Teams-Telefonie.

.NOTES
    File Name : 314_Find-TeamsUsersWithCallForwardingEnabled.ps1
    Author    : Kaffeeundcode
    Version   : 1.0
#>

param(
    [string]$OutputPath = "$PSScriptRoot/314_Find-TeamsUsersWithCallForwardingEnabled.csv",
    [int]$Top = 25,
    [switch]$SkipConnect
)
...
$dataset = @(Get-TtCallingSettingsDataset | Where-Object { [bool](Get-TtProp -InputObject $_ -Path "IsForwardingEnabled") })
