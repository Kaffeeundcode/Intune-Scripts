<#
.SYNOPSIS
    Teams Users With Call Group Members

.DESCRIPTION
    Identifiziert Teams Users With Call Group Members in der Teams-Telefonie.

.NOTES
    File Name : 328_Find-TeamsUsersWithCallGroupMembers.ps1
    Author    : Kaffeeundcode
    Version   : 1.0
#>

param(
    [string]$OutputPath = "$PSScriptRoot/328_Find-TeamsUsersWithCallGroupMembers.csv",
    [int]$Top = 25,
    [switch]$SkipConnect
)
...
$dataset = @(Get-TtCallingSettingsDataset | Where-Object { [int](Get-TtProp -InputObject $_ -Path "CallGroupTargets.Count") -gt 0 })
