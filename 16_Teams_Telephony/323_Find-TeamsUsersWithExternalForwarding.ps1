<#
.SYNOPSIS
    Teams Users With External Forwarding

.DESCRIPTION
    Identifiziert Teams Users With External Forwarding in der Teams-Telefonie.

.NOTES
    File Name : 323_Find-TeamsUsersWithExternalForwarding.ps1
    Author    : Kaffeeundcode
    Version   : 1.0
#>

param(
    [string]$OutputPath = "$PSScriptRoot/323_Find-TeamsUsersWithExternalForwarding.csv",
    [int]$Top = 25,
    [switch]$SkipConnect
)
...
$dataset = @(Get-TtCallingSettingsDataset | Where-Object { [string](Get-TtProp -InputObject $_ -Path "ForwardingTargetType") -match "External|PSTN" })
